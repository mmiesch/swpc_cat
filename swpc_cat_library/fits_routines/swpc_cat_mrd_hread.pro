pro swpc_cat_mrd_hread, unit, header, status, SILENT = silent, FIRSTBLOCK = firstblock, $
...
; found (unless /NO_BADHEADER is set)
  ...
  endwhile
        ...
        if keyword_set(skipdata) then begin 
                bitpix = swpc_cat_fxpar(header,'bitpix')
                naxis  = swpc_cat_fxpar(header,'naxis')
                gcount = swpc_cat_fxpar(header,'gcount') 
                if gcount eq 0 then gcount = 1
                pcount = swpc_cat_fxpar(header,'pcount')
               
                if naxis gt 0 then begin 
                        dims = swpc_cat_fxpar(header,'naxis*')           ;read dimensions
        ndata = product(dims,/integer)
                endif else ndata = 0
                
                nbytes = long64(abs(bitpix) / 8) * gcount * (pcount + ndata)
          swpc_cat_mrd_skip, unit, nbytes
  endif 
  status = 0
  return
error_return:
