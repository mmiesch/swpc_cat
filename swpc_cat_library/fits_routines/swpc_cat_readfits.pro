...
;-
;
function swpc_cat_READFITS, filename, header, heap, CHECKSUM=checksum, $ 
...
;print,'READFITS : filename = ',filename
...
; Get parameters that determine size of data region.
       ...
       bitpix =  swpc_cat_sxpar(header,'BITPIX')
       naxis  = swpc_cat_sxpar(header,'NAXIS')
       gcount = swpc_cat_sxpar(header,'GCOUNT') > 1
       pcount = swpc_cat_sxpar(header,'PCOUNT')
                
       if naxis GT 0 then begin 
            dims = swpc_cat_sxpar( header,'NAXIS*')           ;Read dimensions
            ndata = dims[0]
...
; Check for dummy extension header
 ...
 if Naxis GT 0 then begin 
        Nax = swpc_cat_sxpar( header, 'NAXIS*' )   ;Read NAXES
        ndata = nax[0]
  ...
  if nbytes EQ 0 then begin
        ...
        if do_checksum then begin
             result = swpc_cat_FITS_TEST_CHECKSUM(header, data, ERRMSG = errmsg)
...
; Check for FITS extensions, GROUPS
 ...
 groups = swpc_cat_sxpar( header, 'GROUPS' ) 
...
; to read?
  ...
  if do_checksum then if ((startrow GT 0) or $
   ...
   if exten_no GT 0 then begin
        xtension = strtrim( swpc_cat_sxpar( header, 'XTENSION' , Count = N_ext),2)
   ...
   if (exten_no GT 0) and ((startrow NE 0) or (numrow NE nrow)) then begin
        ...
        nax[1] = nax[1] < numrow
        swpc_cat_sxaddpar, header, 'NAXIS2', nax[1]
        if gzip then begin
    ...
    endelse
        ...
        nax = nax[0:naxis-2]
        swpc_cat_sxdelpar,header,'NAXIS' + strtrim(naxis,2)
        naxis = naxis-1
        swpc_cat_sxaddpar,header,'NAXIS',naxis
        ndata = ndata/lastdim
...
; Read Data in a single I/O call
    ...
    data = make_array( DIM = nax, TYPE = IDL_type, /NOZERO)
     ...
     readu, unit, data
    if unixZ then if not swpc_cat_is_ieee_big() then swpc_cat_ieee_to_host,data
    if (exten_no GT 0) and (pcount GT 0) then begin
        theap = swpc_cat_sxpar(header,'THEAP')
        ...
        readu, unit, heap
        if do_checksum then $
        result = swpc_cat_fits_test_checksum(header,[data,heap],ERRMSG=errmsg)
...
; Use "TEMPORARY" function to speed processing.  
   ...
   if do_scale then begin
          ...
          if bitpix GT 0 then begin
                blank = swpc_cat_sxpar( header, 'BLANK', Count = N_blank) 
                if N_blank GT 0 then $ 
                        blankval = where( data EQ blank, Nblank)
          endif

          Bscale = float( swpc_cat_sxpar( header, 'BSCALE' , Count = N_bscale))
          Bzero = float( swpc_cat_sxpar(header, 'BZERO', Count = N_Bzero ))
...
; Check for unsigned integer (BZERO = 2^15) or unsigned long (BZERO = 2^31)
          ...
          if unsgn then begin
                 swpc_cat_sxaddpar, header, 'BZERO', 0
                 swpc_cat_sxaddpar, header, 'O_BZERO', bzero, $
          ...
          if N_Bscale GT 0  then $ 
               if ( Bscale NE 1. ) then begin
                   ...
                   swpc_cat_sxaddpar, header, 'BSCALE', 1.
                   swpc_cat_sxaddpar, header, 'O_BSCALE', Bscale,' Original BSCALE Value'
               endif
         ...
         if N_Bzero GT 0  then $
               if (Bzero NE 0) then begin
                     ...
                     swpc_cat_sxaddpar, header, 'BZERO', 0.
                     swpc_cat_sxaddpar, header, 'O_BZERO', Bzero,' Original BZERO Value'
               endif
