        FUNCTION swpc_cat_FXPAR, HDR, NAME, ABORT, COUNT=MATCHES, COMMENT=COMMENTS, $
...
;
        IF N_PARAMS() LT 2 THEN BEGIN
            PRINT,'Syntax:  result =  swpc_cat_fxpar( HDR, NAME  [, ABORT ])'
            RETURN, -1
        ENDIF
;
        ...
        IF VECTOR THEN BEGIN
            ...
            IF ( MATCHES GT 0 ) THEN BEGIN
                ...
                FOR I = 0, MATCHES-1 DO         $
                    IF swpc_cat_VALID_NUM( NUMST[I], NUM) THEN NUMBER[I] = NUM
    ...
    IF NOT KEYWORD_SET(NOCONTINUE) THEN BEGIN
                    ...
                    IF (STRLEN(VAL) GT 0) AND $
                      ...
                      (STRMID(HDR[NFOUND[I]+OFF],0,8) EQ 'CONTINUE') THEN BEGIN
                       IF (SIZE(swpc_cat_fxpar(HDR, 'LONGSTRN',/NOCONTINUE)))[1] EQ 7 THEN BEGIN                    
...
;
                        ...
                        VALUE2 = swpc_cat_GETTOK(TEST2,' ')
                        IF swpc_cat_VALID_NUM(VALUE,VAL1) AND swpc_cat_VALID_NUM(VALUE2,VAL2) $
                                THEN BEGIN
...
NOT_COMPLEX:
                        ...
                        VALUE = TEST
                        IF NOT swpc_cat_VALID_NUM(VALUE) THEN GOTO, GOT_VALUE
