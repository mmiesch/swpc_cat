...
; continuation bits.
PRO swpc_cat_FXADDPAR_CONTPAR, VALUE, CONTINUED
...
PRO swpc_cat_FXADDPAR_CONTWARN, HEADER, NAME
...
;  string encoded with the CONTINUE convention.
  ...
  CONTKEY = swpc_cat_FXPAR(HEADER, 'LONGSTRN', COUNT = N_LONGSTRN)
...
;  clean now.
  ...
  swpc_cat_FXADDPAR, HEADER, 'LONGSTRN', 'OGIP 1.0', $
    ' The OGIP long string convention may be used.', $
    BEFORE=NAME

  swpc_cat_FXADDPAR, HEADER, 'COMMENT', $
...
PRO swpc_cat_FXADDPAR, HEADER, NAME, VALUE, COMMENT, BEFORE=BEFORE,      $
...
;
                END ELSE IF NN EQ 'COMMENT ' THEN BEGIN
                        I = swpc_cat_FXPARPOS(KEYWRD,IEND,AFTER=AFTER,BEFORE=BEFORE)
                        IF I EQ IEND THEN I =   $
                            swpc_cat_FXPARPOS(KEYWRD,IEND,AFTER='COMMENT',$
                                     BEFORE='HISTORY')
...
;
                END ELSE BEGIN
                        I = swpc_cat_FXPARPOS(KEYWRD,IEND,AFTER=AFTER,BEFORE=BEFORE)
                        IF I EQ IEND THEN I =   $
                            swpc_cat_FXPARPOS(KEYWRD,IEND,AFTER='',BEFORE='COMMENT')<$
                            swpc_cat_FXPARPOS(KEYWRD,IEND,AFTER='',BEFORE='HISTORY')
                        HEADER[I+1] = HEADER[I:N-2]     ;move rest up
...
;
        IF KEYWRD[0] EQ 'XTENSION' THEN BEGIN
                XTEN = swpc_cat_FXPAR(HEADER,'XTENSION')
...
;
        I = swpc_cat_FXPARPOS(KEYWRD,IEND,AFTER=AFTER,BEFORE=BEFORE)
        IF I EQ IEND THEN I =                                     $
            swpc_cat_FXPARPOS(KEYWRD,IEND,AFTER=AFTER,BEFORE='')         < $
            swpc_cat_FXPARPOS(KEYWRD,IEND,AFTER=AFTER,BEFORE='COMMENT')  < $
            swpc_cat_FXPARPOS(KEYWRD,IEND,AFTER=AFTER,BEFORE='HISTORY')
;
...
;
                        IF NOT KEYWORD_SET(NOCONTINUE) THEN $
                             swpc_cat_fxaddpar_CONTPAR, VAL, CVAL  ELSE $
...
;  more than one element if that is so.
                            ...
                            ;; warning to header
                               ...
                               swpc_cat_FXADDPAR_CONTWARN, HEADER, NAME
                            ENDIF
                            DONE_CONT:
                            RETURN
;
