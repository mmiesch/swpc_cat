pro swpc_cat_fits_add_checksum, hdr, im, no_timestamp = no_timestamp, $
              FROM_IEEE=from_IEEE
;+
; NAME:
;    FITS_ADD_CHECKSUM
...
;-
 ...
 datasum = swpc_cat_sxpar(hdr,'DATASUM', Count = N_DATASUM)
 Nim = N_elements(im)
 datasum_update = 1b
 if Nim GT 1 then begin
     swpc_cat_checksum32,im, dsum,FROM_IEEE = from_IEEE
     remain = Nim mod 2880
     if remain GT 0 then begin
         exten = swpc_cat_sxpar( hdr, 'XTENSION', Count = N_exten)
         if N_exten GT 0 then if exten EQ 'TABLE   ' then $
                 swpc_cat_checksum32,[dsum,replicate(32b,2880-remain)],dsum
    endif
 ...
 if keyword_set(no_timestamp) then tm = '' else swpc_cat_Get_date,tm,/timetag

; Do the Checksum keywords already exist?

  if N_DATASUM GT 0 then verb = 'updated ' else verb = 'created '
  if datasum_update then swpc_cat_sxaddpar,hdr,'DATASUM', sdsum,  $
    ' data unit checksum ' + verb + tm

 test = swpc_cat_sxpar(hdr,'CHECKSUM', Count = N_CHECKSUM)
 if N_CHECKSUM GT 0 then verb = 'updated ' else verb = 'created '
 swpc_cat_sxaddpar,hdr,'CHECKSUM','0000000000000000', $
...
;Make sure each line in header is 80 characters
 ...
 swpc_cat_checksum32,bhdr, hsum, /NoSAVE
 if N_elements(dsum) GT 0 then swpc_cat_checksum32, [dsum,hsum], hdusum $
                        else hdusum = hsum
 
 ch = swpc_cat_FITS_ASCII_ENCODE(not hdusum) ;ASCII encode the complement of the checksum 
 swpc_cat_sxaddpar,hdr,'CHECKSUM',ch
