pro swpc_cat_writefits, filename, data, header, heap, Append = Append,  $
...
;Create a primary or image extension header if not supplied by the user
        ...
        if N_elements(header) LT 2 then begin 
                if keyword_set(append) then swpc_cat_mkhdr, header, data, /IMAGE  $
                                       else swpc_cat_mkhdr, header, data, /EXTEND
        endif else if naxis GT 0 then $         
              swpc_cat_check_fits, data, header, /UPDATE, /FITS
...
; Remove any STSDAS/random group keywords from the primary header
  ...
  if ~keyword_set( APPEND) then begin 
         ...
         swpc_cat_sxdelpar, hdr, [ 'GCOUNT', 'GROUPS', 'PCOUNT', 'PSIZE' ]
  endif
  ...
  if naxis NE 0 then begin
        ...
        if  unsigned then begin
             if type EQ 12 then begin
                     swpc_cat_sxaddpar,hdr,'BZERO',32768,'Data is Unsigned Integer', $
                              before = 'DATE'
                     newdata = fix(data - 32768)
             endif else if type EQ 13 then begin 
                    swpc_cat_sxaddpar,hdr,'BZERO',2147483648,'Data is Unsigned Long', $
                              before = 'DATE'
        ...
        if keyword_set( APPEND) then begin
            ...
            test = file_search(filename, COUNT = n) 
             if n EQ 0 then  begin       ;Create default primary header
                 swpc_cat_mkhdr,h0,0,/exten
                 swpc_cat_writefits,filename,0,h0, checksum = checksum
                 openu, unit, filename, /GET_LUN, /swap_if_little_endian
             endif else begin
            openu, unit, filename, /GET_LUN, /swap_if_little_endian
            swpc_cat_mrd_hread, unit, hprimary
...
; Add any CHECKSUM keywords if desired or already present
    ...
    if ~do_checksum then test = swpc_cat_sxpar(hdr,'CHECKSUM',count=do_checksum)
  
     if do_checksum then begin 
               if N_elements(heap) GT 0 then $
           swpc_cat_fits_add_checksum, hdr, [data,heap] else $
                 swpc_cat_fits_add_checksum, hdr, data
               endline = where( strmid(hdr,0,8) EQ 'END     ', Nend)
       endif
       nmax = endline[0] + 1
...
; Write data
       if naxis EQ 0 then goto, DONE
        bitpix = swpc_cat_sxpar( hdr, 'BITPIX' )
...
; Write optional heap area
        if N_elements(heap) GT 0 then begin
              theap = swpc_cat_sxpar(hdr,'THEAP', Count=N_Theap)
...
; ASCII Tables padded with blanks (32b) otherwise pad with zeros
        if keyword_set( APPEND) then begin
             exten = swpc_cat_sxpar( header, 'XTENSION')
