 function swpc_cat_fits_test_checksum,hdr, data, ERRMSG = errmsg,FROM_IEEE=from_ieee
;+
; NAME:
;    FITS_TEST_CHECKSUM()
; PURPOSE:
...
;-
  ...
  checksum = swpc_cat_sxpar(hdr,'CHECKSUM', Count = N_checksum)
  ...
  swpc_cat_checksum32,ch-48b, sum32
  bhdr = byte(hdr)
  remain = N_elements(bhdr) mod 2880 
  if remain  NE 0 then $
       bhdr = [reform(bhdr,N_elements(bhdr)), replicate(32b, 2880 - remain) ]
  swpc_cat_checksum32,bhdr, hsum, FROM_IEEE = from_ieee, /NOSAVE
  Ndata = N_elements(data)
  if Ndata GT 1 then begin 
           swpc_cat_checksum32, data, dsum, FROM_IEEE= from_ieee
           ...
           swpc_cat_checksum32, [dsum, hsum], hdusum
           dsum = strtrim(dsum,2)
