pro swpc_cat_mkhdr, header, im, naxisx, IMAGE = image, EXTEND = extend
;+
; NAME:
;       MKHDR
; PURPOSE:
...
;-                          
 ...
 if keyword_set( IMAGE) then $
    swpc_cat_sxaddpar, header, 'XTENSION', 'IMAGE   ',' IMAGE extension' $
 else $
    swpc_cat_sxaddpar, header, 'SIMPLE', 'T',' Written by IDL:  '+ systime()

 swpc_cat_sxaddpar, header, 'BITPIX', bitpix, ' Number of bits per data pixel'
 swpc_cat_sxaddpar, header, 'NAXIS', S[0],' Number of data axes'       ;# of dimensions

 if ( s[0] GT 0 ) then begin
   for i = 1, s[0] do swpc_cat_sxaddpar,header,'NAXIS' + strtrim(i,2),s[i]
 endif

 if keyword_set( IMAGE) then begin
     swpc_cat_sxaddpar, header, 'PCOUNT', 0, ' No Group Parameters'
     swpc_cat_sxaddpar, header, 'GCOUNT', 1, ' One Data Group'
 endif else begin
     if keyword_set( EXTEND) or (s[0] EQ 0) then $
          swpc_cat_sxaddpar, header, 'EXTEND', 'T', ' FITS data may contain extensions'
     swpc_cat_Get_date, dte                       ;Get current date as CCYY-MM-DD
     swpc_cat_sxaddpar, header, 'DATE', dte, $
           ' Creation UTC (CCCC-MM-DD) date of FITS header'
  endelse

 if stype EQ 12 then swpc_cat_sxaddpar, header,'O_BZERO',32768, $
            ' Original Data is Unsigned Integer'
 if stype EQ 13 then swpc_cat_sxaddpar, header,'O_BZERO',2147483648, $
 ...
 if not keyword_set(IMAGE) then begin   ;Add FITS definition for primary header
     swpc_cat_sxaddpar,header,'COMMENT ', $
