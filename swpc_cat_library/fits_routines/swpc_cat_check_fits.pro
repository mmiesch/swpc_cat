pro swpc_cat_check_FITS, im, hdr, dimen, idltype, UPDATE = update, NOTYPE = notype, $
...
;- 
 ...
 nax = swpc_cat_fxpar( hdr, 'NAXIS', Count = N_naxis ) 
...
BITPIX:     
 ...
 if ~keyword_set( NOTYPE ) then begin
  ...
  bitpix = swpc_cat_fxpar( hdr, 'BITPIX')
...
BITPIX_ERROR:
    ...
    swpc_cat_fxaddpar, hdr, 'BITPIX', bitpix, comment
    return
...
DIMEN_ERROR:
   if keyword_set( UPDATE ) then begin
        swpc_cat_fxaddpar, hdr, 'NAXIS', ndimen, before = 'NAXIS1'
  naxis = 'NAXIS' + strtrim(indgen(ndimen)+1,2)
        for i = 1, ndimen do swpc_cat_fxaddpar, hdr, naxis[i-1], dimen[i-1], $
                'Number of positions along axis ' + strtrim(i,2), $
                after = 'NAXIS' + strtrim(i-1,2)          
        if naxi GT ndimen then begin
                for i = ndimen+1, naxi do swpc_cat_sxdelpar, hdr, 'NAXIS'+strtrim(i,2)
        endif
        if ~keyword_set(SILENT) then message, /INF, $
