function swpc_cat_SXPAR, hdr, name, abort, COUNT=matches, COMMENT = comments, $
        ...
        if vector then begin
            ...
            if ( matches gt 0 ) then begin
                ...
                for i = 0, matches-1 do         $
                    if swpc_cat_VALID_NUM( numst[i], num,/INTEGER) then number[i] = num
    ...
    if not keyword_set(nocontinue) then begin
                ...
                if (strlen(val) gt 0) and $
                  ...
                  (strmid(hdr[nfound[i]+off],0,8) EQ 'CONTINUE') then begin
                   if (size(swpc_cat_sxpar(hdr, 'LONGSTRN',/NoCONTINUE)))[1] EQ 7 then begin                    
